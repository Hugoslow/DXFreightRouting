...
def setup_database():
    ...
    try:
        ...
        # Temporary data upload page - DELETE AFTER USE
@app.get("/upload-data")
def upload_data_page(request: Request):
    html = """
    <html>
    <head><title>Upload Data</title></head>
    <body style="font-family: Arial; max-width: 600px; margin: 50px auto; padding: 20px;">
        <h1>Upload Data</h1>
        
        <h3>Step 1: Upload Depots</h3>
        <form action="/upload-depots" method="post" enctype="multipart/form-data">
            <input type="file" name="file" accept=".xlsx" required>
            <button type="submit">Upload Depots</button>
        </form>
        
        <h3>Step 2: Upload Collection Points</h3>
        <form action="/upload-cps" method="post" enctype="multipart/form-data">
            <input type="file" name="file" accept=".xlsx" required>
            <button type="submit">Upload CPs</button>
        </form>
        
        <h3>Step 3: Calculate Distances</h3>
        <form action="/calculate-distances" method="post">
            <button type="submit">Calculate All Distances</button>
        </form>
...
@app.post("/upload-depots")
async def upload_depots(file: UploadFile = File(...)):
    import pandas as pd
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            count += 1
        db.commit()
        return {"message": f"Uploaded {count} depots successfully!"}
    finally:
        db.close()


@app.post("/upload-cps")
async def upload_cps(file: UploadFile = File(...)):
    import pandas as pd
    from io import BytesIO
    from app.database import SessionLocal
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            count += 1
        db.commit()
        return {"message": f"Uploaded {count} collection points successfully!"}
    finally:
        db.close()
